...
class SubscribeEmailService:
    ...
    async def start_consumer_loop(self):
        await self.consumer.subscribe(topics=self.topics)
        ...
        while self.is_running:
            ...
            if msg.error():
                if msg.error().code == KafkaError._PARTITION_EOF:
                    ...
                    logger.error(f"consumer error: {msg.error()}") # TODO: push failed to dead letter queue by calling producer again
                    raise KafkaException(msg.error()) # or continue
